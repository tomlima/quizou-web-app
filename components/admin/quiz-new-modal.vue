...
async function saveQuizDraft(){
  ...
  const tagObject:Tag = tagStore.getTagByName(tag.value); 
  ...
  const resultOfQuizCreation:string | null =  await quizStore.createQuiz(quizObj);
  console.info(resultOfQuizCreation);
